...
public class WarehouseConcurrencyIT {
  ...
  public void testConcurrentReadsAreNonBlocking() throws InterruptedException {
    // Create a warehouse first (commits immediately, since createWarehouseUseCase.create()
    // has no ambient transaction here) so the reader threads on separate connections can see it.
    Warehouse warehouse = new Warehouse();
